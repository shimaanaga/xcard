...
<head>
    <title> XCard | {{ !empty($title)?$title:_i('admin Reset Password') }}</title>
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="login-card card-block auth-body">
                    ...
                    @endif
{{--                        @dd($check_token->token)--}}
                        <form class="md-float-material" method="POST" action="{{ aUrl('resetPassword/'. $check_token->token) }}">
                        {!! csrf_field() !!}
                        {{method_field('post')}}
                        ...
                        <div class="auth-box">
                            <div class="row m-b-20">
                                <div class="col-md-12">
                                    <h3 class="text-left">{{ _I('Reset your Password ? ') }}</h3>
                                </div>
                            </div>
                            <p class="text-inverse b-b-default text-right">{{ _i('Back to ') }}<a href="{{ aUrl('login') }}">{{ _i('Login.') }}</a></p>
                            <div class="input-group">
                                <input type="email" name="email" required value="{{ $check_token->email }}" class="form-control" placeholder="{{ _i('email') }}">
                                <span class="md-line"></span>
                            </div>
                            <div class="input-group">
                                <input type="password" name="password" required class="form-control" placeholder="{{ _i('Your New Password') }}" autocomplete="off">
                                <span class="md-line"></span>
                            </div>
                            <div class="input-group">
                                <input type="password" name="password_confirmation" class="form-control" placeholder="{{ _i('Confirm Your Password') }}" autocomplete="off">
                                <span class="md-line"></span>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <button type="submit" class="btn btn-primary btn-md btn-block waves-effect text-center m-b-20">{{ _i('Save') }}</button>
                                </div>
                            </div>
                            <hr>
                            <div class="row">
